          ...
          <PopoverTrigger
            render={
              <button
                type="button"
                className={cn(
                  'relative inline-flex items-center justify-center size-7 rounded-full select-none text-white text-xs font-semibold shrink-0 ring-2 ring-background hover:scale-105 transition-transform cursor-pointer shadow-xs',
                  className,
                )}
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-6 gap-2">
              {COLLAB_PALETTE_COLORS.map((color: string) => (
                <button
                  key={color}
                  type="button"
